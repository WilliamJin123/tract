...
<task type="auto">
  ...
  <action>
    **session.py** -- Create new file with the `Session` class:
    ...
    Spawn/collapse:
    - `spawn(self, parent: Tract, *, purpose: str, inheritance: str = "head_snapshot", display_name: str | None = None, max_tokens: int | None = None) -> Tract`: Delegate to `spawn_tract()` from operations/spawn.py. `max_tokens` caps the size of a head_snapshot (ignored for full_clone). Returns the new child Tract.
    ...
    - `get_child_tract(self, collapse_commit_hash: str) -> Tract`: Read collapse metadata from the commit (`collapse_source_tract_id`), open and return the child Tract. Raises SessionError if the commit has no collapse metadata or child tract not found. Implements the "expand for debugging" ROADMAP success criteria.
    ...
    **operations/spawn.py** -- Create new file with spawn and collapse operations:
    ...
    2. `_head_snapshot(parent_tract, child_commit_engine, *, max_tokens: int | None = None) -> str`:
       - Compile parent's context: `parent_tract.compile()`
       - Format compiled messages into a single text blob (with role prefixes: `[system]: ...\n[user]: ...\n[assistant]: ...`)
       - If `max_tokens` is provided and the compiled text exceeds it, truncate to fit the budget (truncate from the oldest messages first, preserving the most recent context)
    ...
    4. `collapse_tract(parent_tract, child_tract, spawn_repo, *, content, instructions, auto_commit, target_tokens, llm_client) -> CollapseResult`:
       ...
       - If `auto_commit` is False or None: return CollapseResult without committing (caller reviews summary_text and commits manually)
       - Return CollapseResult(parent_commit_hash=commit_hash_or_None, child_tract_id, summary_text=summary, summary_tokens, source_tokens, purpose)
       - NOTE: `summary_text` is always populated (both auto_commit=True and False) so the caller can inspect the summary. `parent_commit_hash` is None when auto_commit=False (no commit created yet).
...
<task type="auto">
  ...
  <action>
    **operations/session_ops.py** -- Create new file with cross-repo query functions:
    ...
    3. `search(session, blob_repo, term: str, *, tract_id: str | None = None) -> list[CommitInfo]`:
       - Escape LIKE wildcards in term: `escaped = term.replace('%', r'\%').replace('_', r'\_')`
       - Query BlobRow where payload_json LIKE '%{escaped}%' with escape char
       - Join with CommitRow to get commit info
       - Filter by tract_id if provided
       - Convert to CommitInfo and return
       - Use `BlobRow.payload_json.like(f"%{escaped}%", escape='\\')` for the LIKE query
    ...
    **tests/test_session.py** -- Create test file (~20 tests) covering Session and cross-repo queries:
    ...
    4. Concurrent multi-threaded tests (~3 tests):
       - `test_concurrent_commits_from_different_threads`: create 2 tracts, each in its own thread via `concurrent.futures.ThreadPoolExecutor`, both commit 10 messages simultaneously, verify all 20 commits exist after joining
       - `test_read_while_write`: one thread commits messages in a loop, another thread calls timeline() repeatedly, verify no errors and timeline returns increasing counts
       - `test_concurrent_spawn_and_commit`: one thread spawns a child, another thread commits to the parent simultaneously, verify both operations succeed without database locked errors

    5. Expand for debugging tests (~2 tests):
       - `test_get_child_tract`: collapse a child, then call session.get_child_tract(collapse_commit_hash), verify it returns the child Tract with all its commits accessible
       - `test_get_child_tract_invalid_commit`: call get_child_tract with a non-collapse commit hash, verify SessionError is raised
  ...
  <verify>
    ...
    Run `python -m pytest tests/test_session.py -v` -- all ~25 session/query/concurrent tests pass.
    Run `python -m pytest tests/ -x -q` -- all tests pass (586 existing + ~50 new = ~636 total).
...
8. session.get_child_tract() navigates from collapse commit back to child tract (expand for debugging)
9. ~50 new tests pass (including concurrent thread tests); ~586 existing tests have zero regressions
</success_criteria>
