...
<task type="auto">
  ...
  <action>
    **models/session.py** -- Create new file with:
    ...
    3. `CollapseResult` -- frozen dataclass for collapse operation results:
       - `parent_commit_hash: str` (the summary commit created in the parent)
       - `child_tract_id: str`
       - `summary_tokens: int`
       - `source_tokens: int`
       - `purpose: str`
  ...
  <verify>
    Run `python -m pytest tests/test_spawn_storage.py -v` -- all ~23 new tests pass.
    Run `python -m pytest tests/ -x -q` -- all existing tests plus new tests pass (total ~586).
...
<verification>
1. `python -m pytest tests/ -x -q` -- all tests pass (563 existing + ~23 new)
...
7. ~23 new tests pass; 563 existing tests have zero regressions
</success_criteria>
