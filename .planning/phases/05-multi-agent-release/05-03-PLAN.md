  ...
  <action>
    **pyproject.toml** -- Update package configuration for release:
    ...
    3. Add complete project metadata:
       ...
       ```
    4. Add project URLs (consistent with tract-ai distribution name):
       ```toml
       [project.urls]
       Homepage = "https://github.com/WilliamJin123/tract"
       Documentation = "https://github.com/WilliamJin123/tract#readme"
       Repository = "https://github.com/WilliamJin123/tract"
       ```
    ...
    **src/tract/py.typed** -- Create empty marker file for PEP 561 compliance:
    - Create an empty file at `src/tract/py.typed` so that downstream type checkers (mypy, pyright) process the package's type information.
...
<task type="auto">
  ...
  <action>
    ...
    **tests/test_integration_multiagent.py** -- Create end-to-end integration test file (~15 tests):
    ...
    16. `test_cross_session_handoff`:
        - Agent A creates a tract, commits work, then commits SessionContent(session_type="end", summary="Built backend", decisions=["Chose FastAPI"], next_steps=["Add frontend"])
        - Close session, reopen
        - Agent B spawns from Agent A's final state (head_snapshot inheritance)
        - Verify Agent B's initial context includes Agent A's session boundary commit
        - Verify Agent B can access Agent A's decisions and next_steps from the compiled context
  ...
  <verify>
    Run `python -m pytest tests/test_integration_multiagent.py -v` -- all ~16 integration tests pass.
    Run `python -m pytest tests/ -x -q` -- all tests pass (~636 existing + ~16 new = ~652 total).
...
5. ~16 end-to-end integration tests cover the complete multi-agent workflow including cross-session handoff
6. All ~652 tests pass with zero regressions
7. Version is 0.1.0
</success_criteria>
